...
public class AnimationFileReader {
  ...
  public <T> T readFile(String fileName, TweenModelBuilder<T> builder) throws
    ...
    while (sc.hasNext()) {
      ...
      switch (command) {
        case "layer":
          GroupInfo ginfo = readGroupInfo(sc);
          builder.addGroupFigs(ginfo.layer, ginfo.figs);
          break;
        case "rectangle":
        ...
        case "rotate":
          ...
          builder.addRotation(rotateInfo.name,
  ...
  private GroupInfo readGroupInfo(Scanner sc) throws
          IllegalStateException, InputMismatchException {
  ...
  class GroupInfo extends Inputable {
    private int layer;
    private ArrayList<String> figs = new ArrayList<>();

    GroupInfo() {
      super();
      valueFlags.put("layer", false);
      valueFlags.put("figs", false);
      valueFlags.put("close", false);
    }
    ...
    void setFigs(String fig) {
      this.figs.add(fig);
      valueFlags.replace("figs", true);
    }

    void setClose() {
      valueFlags.replace("close", true);
    }
